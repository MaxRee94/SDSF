...
public:
	...
	void compute_no_seeds(Tree &tree, float fecundity_multiplier) {
		total_no_seeds_produced = 49324 * exp(0.0491 * tree.dbh);		// Fitted to averages generated by model: #seedlings = STR' * (dbh / 30)^alpha, modified from Uriarte et al(2005),
		...
		total_no_seeds_produced *= 0.0771 * exp(-4.956 * (strategy.seed_mass / 3.56));		// Re-introduce (normalized) seed mass influence.
		total_no_seeds_produced *= fecundity_multiplier;
