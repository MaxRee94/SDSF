...
public:
	...
	void compute_no_seeds(Tree &tree, float fecundity_multiplier) {
		total_seeds_produced = 49324 * exp(0.0491 * tree.dbh);		// Fitted to averages generated by model: #seedlings = STR' * (dbh / 30)^alpha, modified from Uriarte et al(2005),
		...
		total_seeds_produced *= 0.0771 * exp(-4.956 * (strategy.seed_mass / 3.56));		// Re-introduce (normalized) seed mass influence.
		total_seeds_produced *= fecundity_multiplier;
		no_seeds = (float)total_seeds_produced * strategy.recruitment_probability;
	}
	...
	void compute_fruit_abundance(float fecundity_multiplier) {
		...
		fruit_abundance = total_seeds_produced / strategy.no_seeds_per_diaspore;
	}
	void update(Tree& tree, float fecundity_multiplier) {
		compute_no_seeds(tree, fecundity_multiplier);
		compute_no_diaspora();
		compute_fruit_abundance(fecundity_multiplier);
	}
	int no_seeds = 0; // Number of seeds dispersed.
	int no_diaspora = 0; // Number of diaspora dispersed.
	int fruit_abundance = 0;
	int total_seeds_produced = 0; // Number of seeds produced (always equal- or higher than no_seeds).
	float seed_mass = 0;
	Strategy strategy;
