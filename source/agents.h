...
public:
	...
	void compute_no_seeds(Tree &tree, float environmental_recruitment_factor) {
		...
		//printf("no seeds: %i (before recruitment prob), dbh %f \n", no_seeds, tree.dbh);
		no_seeds *= strategy.recruitment_probability;
		//printf("no seeds: %i (after recruitment prob (which is %f)) \n", no_seeds, strategy.recruitment_probability);
		no_seeds *= environmental_recruitment_factor;
	}
	...
	void update(Tree& tree, float environmental_recruitment_factor) {
		compute_no_seeds(tree, environmental_recruitment_factor);
		compute_no_diaspora();
	}
	int no_seeds = 0;
	int no_diaspora = 0;
