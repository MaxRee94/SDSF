...
public:
	...
	Tests(int _timestep, float _cell_width, float _self_ignition_factor, float _rainfall, float _seed_bearing_threshold,
		...
		float _resource_grid_relative_size, float _mutation_rate, float _environmental_recruitment_factor, int _verbosity, int grid_size, float dbh_q1, float dbh_q2
	) {
		dynamics = Dynamics(_timestep, _cell_width, _self_ignition_factor, _rainfall, _seed_bearing_threshold,
			...
			_resource_grid_relative_size, _mutation_rate, _environmental_recruitment_factor, _verbosity
		);
